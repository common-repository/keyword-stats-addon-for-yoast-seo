<?php
/*
Plugin Name: Keyword Stats Addon for Yoast SEO
...
define('KS_PAGE_DIR', plugin_dir_path(__FILE__).'pages/');
define('KS_INCLUDE_URL', plugin_dir_url(__FILE__).'includes/');

//Include menu and assign page
function ksysa_plugin_menu() {
    $icon = KS_URL. 'includes/icon.png';
	add_menu_page("Keyword Stats", "Keyword Stats", "administrator", "keyword-stats", "ksysa_plugin_pages", $icon ,30);
	add_submenu_page("keyword-stats", "About Us", "About Us", "administrator", "about-us", "ksysa_plugin_pages");
}
add_action("admin_menu", "ksysa_plugin_menu");

function ksysa_plugin_pages() {

   $itm = KS_PAGE_DIR.$_GET["page"].'.php';
   include($itm);
}

//add admin css
function ksysa_admin_css() {
  wp_register_style('admin_css', plugins_url('includes/admin-style.css',__FILE__ ));
  wp_enqueue_style('admin_css');
}
add_action( 'admin_init','ksysa_admin_css');

//function for pagination
 function ksysa_pagination($pages = '', $range = 4)
{ 
     $showitems = ($range * 2)+1; 
     //global $paged;
...
function ksysa_get_post_count( $kword){
